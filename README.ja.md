...
TurboWarpアプリの初期雛形です。用途固有の機能は未実装です。
...
- **起動確認**：SB3をダウンロードしてTurboWarpで開き、緑の旗で起動状態を確認します。
...
空リポジトリをcloneし、config/app.json形式の設定JSONを用意して実行します。既存.gitを保持し、他のファイルがある場合は拒否します。
